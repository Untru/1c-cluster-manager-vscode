...
test("translates safe infobase registration lifecycle without dropping a database", async (t) => {
  ...
  assert.equal((await fetch(`${root}/infobases/${infobaseId}/settings`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionsDeny: true, scheduledJobsDeny: true, licenseDistribution: "deny" }) })).status, 200);
